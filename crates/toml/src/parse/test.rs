use common::Pos;
use pretty_assertions::assert_eq;

use super::*;
use crate::datetime::{DateTimeField, Offset, Time};
use crate::map::MapInner;
use crate::test::*;

#[track_caller]
fn check<'a, const SIZE: usize>(
    input: &str,
    expected_builder: impl FnOnce(&mut AstBuilder<'a>) -> [Toplevel<'a>; SIZE],
) {
    let mut ctx = TomlDiagnostics::default();
    let bump = Bump::new();
    let tokens = ctx.lex(&bump, input);
    let ast = ctx.parse(&bump, tokens);

    // HACK
    let expected_bump = Box::leak(Box::new(Bump::new()));
    let mut builder = AstBuilder::new(expected_bump);
    let expected_toplevel = expected_builder(&mut builder);
    assert_eq!(
        expected_toplevel, ast.toplevel,
        "\nerrors: {:#?}\nwarnings: {:#?}",
        ctx.errors, ctx.warnings
    );
    assert_eq!(
        builder.comments, ast.comments,
        "\nerrors: {:#?}\nwarnings: {:#?}",
        ctx.errors, ctx.warnings
    );
    assert_eq!(Vec::<Error>::new(), ctx.errors);
    assert_eq!(Vec::<Warning>::new(), ctx.warnings);
}

#[track_caller]
fn check_error<'a, const SIZE: usize>(
    input: &str,
    expected_builder: impl FnOnce(&mut AstBuilder<'a>) -> [Toplevel<'a>; SIZE],
    error: Error,
) {
    let mut ctx = TomlDiagnostics::default();
    let bump = Bump::new();
    let tokens = ctx.lex(&bump, input);
    let ast = ctx.parse(&bump, tokens);

    // HACK
    let expected_bump = Box::leak(Box::new(Bump::new()));
    let mut builder = AstBuilder::new(expected_bump);
    let expected_toplevel = expected_builder(&mut builder);

    assert_eq!(
        expected_toplevel, ast.toplevel,
        "\nerrors: {:#?}\nwarnings: {:#?}",
        ctx.errors, ctx.warnings
    );
    assert_eq!(
        builder.comments, ast.comments,
        "\nerrors: {:#?}\nwarnings: {:#?}",
        ctx.errors, ctx.warnings
    );
    assert_eq!(vec![error], ctx.errors);
    assert_eq!(Vec::<Warning>::new(), ctx.warnings);
}

#[test]
fn float_special_values() {
    let input = "
    a0 = nan
    a1 = +nan
    a2 = -nan

    b0 = inf
    b1 = +inf
    b2 = -inf

    c0 = +0.0
    c1 = -0.0
";

    let (_, table) = parse_simple(input);

    assert!(expect_float(&table, "a0").is_nan());
    assert!(expect_float(&table, "a0").is_sign_positive());
    assert!(expect_float(&table, "a1").is_nan());
    assert!(expect_float(&table, "a1").is_sign_positive());
    assert!(expect_float(&table, "a2").is_nan());
    assert!(expect_float(&table, "a2").is_sign_negative());

    assert!(expect_float(&table, "b0").is_infinite());
    assert!(expect_float(&table, "b0").is_sign_positive());
    assert!(expect_float(&table, "b1").is_infinite());
    assert!(expect_float(&table, "b1").is_sign_positive());
    assert!(expect_float(&table, "b2").is_infinite());
    assert!(expect_float(&table, "b2").is_sign_negative());

    assert_eq!(expect_float(&table, "c0"), 0.0);
    assert!(expect_float(&table, "c0").is_sign_positive());
    assert_eq!(expect_float(&table, "c1"), 0.0);
    assert!(expect_float(&table, "c1").is_sign_negative());
}

#[test]
fn assign_negative_int() {
    check_simple(
        "num = -2",
        MapInner::from_iter([("num".into(), SimpleVal::Int(-2))]),
    );
}

#[test]
fn assign_positive_int() {
    check_simple(
        "num = +83",
        MapInner::from_iter([("num".into(), SimpleVal::Int(83))]),
    );
}

#[test]
fn assign_sgined_zero_ints() {
    check_simple(
        "num = 0",
        MapInner::from_iter([("num".into(), SimpleVal::Int(0))]),
    );
    check_simple(
        "num = -0",
        MapInner::from_iter([("num".into(), SimpleVal::Int(0))]),
    );
    check_simple(
        "num = +0",
        MapInner::from_iter([("num".into(), SimpleVal::Int(0))]),
    );
}

#[test]
fn assign_sgined_zero_floats() {
    check_simple(
        "num = 0.0",
        MapInner::from_iter([("num".into(), SimpleVal::Float(0.0))]),
    );
    check_simple(
        "num = -0.0",
        MapInner::from_iter([("num".into(), SimpleVal::Float(-0.0))]),
    );
    check_simple(
        "num = +0.0",
        MapInner::from_iter([("num".into(), SimpleVal::Float(0.0))]),
    );
}

#[test]
fn sign_prefixed_binary_int() {
    check_error(
        "num = -0b10100",
        |b| {
            [Toplevel::Assignment(tainvalid(
                b.ec(1),
                0,
                "num",
                "-0b10100",
            ))]
        },
        Error::PrefixedIntSignNotAllowed(Pos { line: 0, char: 6 }),
    );
    check_error(
        "num = +0b10100",
        |b| {
            [Toplevel::Assignment(tainvalid(
                b.ec(1),
                0,
                "num",
                "+0b10100",
            ))]
        },
        Error::PrefixedIntSignNotAllowed(Pos { line: 0, char: 6 }),
    );
}

#[test]
fn sign_prefixed_octal_int() {
    check_error(
        "num = -0o361",
        |b| [Toplevel::Assignment(tainvalid(b.ec(1), 0, "num", "-0o361"))],
        Error::PrefixedIntSignNotAllowed(Pos { line: 0, char: 6 }),
    );
    check_error(
        "num = +0o361",
        |b| [Toplevel::Assignment(tainvalid(b.ec(1), 0, "num", "+0o361"))],
        Error::PrefixedIntSignNotAllowed(Pos { line: 0, char: 6 }),
    );
}

#[test]
fn signed_prefixed_hexadecimal_int() {
    check_error(
        "num = -0xc20",
        |b| [Toplevel::Assignment(tainvalid(b.ec(1), 0, "num", "-0xc20"))],
        Error::PrefixedIntSignNotAllowed(Pos { line: 0, char: 6 }),
    );
    check_error(
        "num = +0xc20",
        |b| [Toplevel::Assignment(tainvalid(b.ec(1), 0, "num", "+0xc20"))],
        Error::PrefixedIntSignNotAllowed(Pos { line: 0, char: 6 }),
    );
}

#[test]
fn uppercase_binray_radix_not_allowed() {
    check_simple_error(
        "num = 0B10",
        MapInner::from_iter([("num".into(), SimpleVal::Invalid("0B10".into()))]),
        Error::UppercaseIntRadix(IntPrefix::Binary, Pos::new(0, 7)),
    );
}

#[test]
fn uppercase_octal_radix_not_allowed() {
    check_simple_error(
        "num = 0O10",
        MapInner::from_iter([("num".into(), SimpleVal::Invalid("0O10".into()))]),
        Error::UppercaseIntRadix(IntPrefix::Octal, Pos::new(0, 7)),
    );
}

#[test]
fn uppercase_hexadecimal_radix_not_allowed() {
    check_simple_error(
        "num = 0X10",
        MapInner::from_iter([("num".into(), SimpleVal::Invalid("0X10".into()))]),
        Error::UppercaseIntRadix(IntPrefix::Hexadecimal, Pos::new(0, 7)),
    );
}

#[test]
fn assign_bool() {
    check("abc = false", |b| {
        [Toplevel::Assignment(tabool(b.ec(1), 0, "abc", false))]
    });
}

#[test]
fn assign_float() {
    check("abc = 23.5", |b| {
        [Toplevel::Assignment(tafloat(b.ec(1), 0, "abc", "23.5"))]
    });
}

#[test]
fn assign_float_with_exp1() {
    check("abc = 23.5e+9", |b| {
        [Toplevel::Assignment(tafloat(b.ec(1), 0, "abc", "23.5e+9"))]
    });
}

#[test]
fn assign_float_with_exp2() {
    check("abc = 23.5e-1_2", |b| {
        [Toplevel::Assignment(tafloat(
            b.ec(1),
            0,
            "abc",
            "23.5e-1_2",
        ))]
    });
}

#[test]
fn float_fractional_part_ends_with_underscore() {
    check_error(
        "abc = 23.5_e9",
        |b| {
            [Toplevel::Assignment(tainvalid(
                b.ec(1),
                0,
                "abc",
                "23.5_e9",
            ))]
        },
        Error::LitEndsWithUnderscore(LitPart::FloatFract, Pos { line: 0, char: 10 }),
    );
}

#[test]
fn int_with_underscore() {
    check("abc = 1_000", |b| {
        [Toplevel::Assignment(taint(b.ec(1), 0, "abc", "1_000"))]
    });
}

#[test]
fn invalid_prefixed_int_radix() {
    check_error(
        "abc = 0c324",
        |b| [Toplevel::Assignment(tainvalid(b.ec(1), 0, "abc", "0c324"))],
        Error::ExpectedRadixOrDateTime(FmtChar('c'), Pos { line: 0, char: 7 }),
    );
}

#[test]
fn prefixed_int_digit_too_big() {
    check_error(
        "abc = 0o384",
        |b| [Toplevel::Assignment(tainvalid(b.ec(1), 0, "abc", "0o384"))],
        Error::IntDigitTooBig(IntPrefix::Octal, FmtChar('8'), Pos { line: 0, char: 9 }),
    );
}

#[test]
fn prefixed_int_starts_with_underscore() {
    check_error(
        "abc = 0o_43",
        |b| [Toplevel::Assignment(tainvalid(b.ec(1), 0, "abc", "0o_43"))],
        Error::PrefixedIntValueStartsWithUnderscore(Pos { line: 0, char: 8 }),
    );
}

#[test]
fn prefixed_int_ends_with_underscore() {
    check_error(
        "abc = 0o43_",
        |b| [Toplevel::Assignment(tainvalid(b.ec(1), 0, "abc", "0o43_"))],
        Error::PrefixedIntValueEndsWithUnderscore(Pos { line: 0, char: 10 }),
    );
}

#[test]
fn dotted_key() {
    check("a.b.c = false", |builder| {
        [Toplevel::Assignment(twrap(
            builder.empty_comments(1),
            Assignment {
                key: Key::Dotted(builder.bump.alloc([
                    DottedIdent {
                        ident: Ident::from_plain_lit(
                            "a",
                            Span::from_pos_len(Pos { line: 0, char: 0 }, 1),
                        ),
                        dot: Some(Pos { line: 0, char: 1 }),
                    },
                    DottedIdent {
                        ident: Ident::from_plain_lit(
                            "b",
                            Span::from_pos_len(Pos { line: 0, char: 2 }, 1),
                        ),
                        dot: Some(Pos { line: 0, char: 3 }),
                    },
                    DottedIdent {
                        ident: Ident::from_plain_lit(
                            "c",
                            Span::from_pos_len(Pos { line: 0, char: 4 }, 1),
                        ),
                        dot: None,
                    },
                ])),
                eq: Pos { line: 0, char: 6 },
                val: bool(0, 8, false),
            },
        ))]
    });
}

#[test]
fn int_identifier() {
    check("123 = false", |b| {
        [Toplevel::Assignment(tabool(b.ec(1), 0, "123", false))]
    });
}

#[test]
fn invalid_int_identifier() {
    check_error(
        "+99 = false",
        |b| [Toplevel::Assignment(tabool(b.ec(1), 0, "+99", false))],
        Error::InvalidCharInIdentifier(FmtChar('+'), Pos { line: 0, char: 0 }),
    );
}

#[test]
fn space_between_array_header_brackets() {
    check_simple_error(
        "[ [a.b]]",
        MapInner::from_iter([(
            "a".into(),
            SimpleVal::Table(MapInner::from_iter([(
                "b".into(),
                SimpleVal::Array(vec![SimpleVal::Table(MapInner::new())]),
            )])),
        )]),
        Error::SpaceBetweenArrayPars(Span::from_pos_len(Pos { line: 0, char: 1 }, 1)),
    );

    check_simple_error(
        "[[a.b] ]",
        MapInner::from_iter([(
            "a".into(),
            SimpleVal::Table(MapInner::from_iter([(
                "b".into(),
                SimpleVal::Array(vec![SimpleVal::Table(MapInner::new())]),
            )])),
        )]),
        Error::SpaceBetweenArrayPars(Span::from_pos_len(Pos { line: 0, char: 6 }, 1)),
    );
}

#[test]
fn invalid_float_literal_as_identifier() {
    check_error(
        "23e+3 = 'hello'",
        |b| {
            [Toplevel::Assignment(tastring(
                b.ec(1),
                0,
                "23e+3",
                "'hello'",
                Quote::Literal,
            ))]
        },
        Error::InvalidCharInIdentifier(FmtChar('+'), Pos { line: 0, char: 3 }),
    );
}

#[test]
fn multi_line_literal_string_identifier() {
    check_error(
        "'''abc''' = false",
        |_| [],
        Error::MultilineLiteralStringIdent(Span::from_pos_len(Pos::new(0, 0), 9)),
    );
}

#[test]
fn multi_line_basic_string_identifier() {
    check_error(
        r#""""abc""" = false"#,
        |_| [],
        Error::MultilineBasicStringIdent(Span::from_pos_len(Pos::new(0, 0), 9)),
    );
}

#[test]
fn inline_array() {
    check("array = [0, 1, 2]", |builder| {
        [Toplevel::Assignment(ta(
            builder.empty_comments(1),
            0,
            "array",
            Value::InlineArray(InlineArray {
                comments: builder.empty_comments(2),
                l_par: Pos { line: 0, char: 8 },
                values: vec![
                    InlineArrayValue {
                        comments: builder.empty_comments(3),
                        val: int(0, 9, "0"),
                        comma: Some(Pos { line: 0, char: 10 }),
                    },
                    InlineArrayValue {
                        comments: builder.empty_comments(3),
                        val: int(0, 12, "1"),
                        comma: Some(Pos { line: 0, char: 13 }),
                    },
                    InlineArrayValue {
                        comments: builder.empty_comments(3),
                        val: int(0, 15, "2"),
                        comma: None,
                    },
                ],
                end: End::Par(Pos { line: 0, char: 16 }),
            }),
        ))]
    });
}

#[test]
fn multi_line_inline_array() {
    check("array = [\n  0,\n  1,\n  2,\n]", |builder| {
        [Toplevel::Assignment(ta(
            builder.empty_comments(1),
            0,
            "array",
            Value::InlineArray(InlineArray {
                comments: builder.empty_comments(2),
                l_par: Pos { line: 0, char: 8 },
                values: vec![
                    InlineArrayValue {
                        comments: builder.empty_comments(3),
                        val: int(1, 2, "0"),
                        comma: Some(Pos { line: 1, char: 3 }),
                    },
                    InlineArrayValue {
                        comments: builder.empty_comments(3),
                        val: int(2, 2, "1"),
                        comma: Some(Pos { line: 2, char: 3 }),
                    },
                    InlineArrayValue {
                        comments: builder.empty_comments(3),
                        val: int(3, 2, "2"),
                        comma: Some(Pos { line: 3, char: 3 }),
                    },
                ],
                end: End::Par(Pos { line: 4, char: 0 }),
            }),
        ))]
    });
}

#[test]
fn inline_array_recover_comma() {
    check_error(
        "array = [0, 1  2]",
        |builder| {
            [Toplevel::Assignment(ta(
                builder.empty_comments(1),
                0,
                "array",
                Value::InlineArray(InlineArray {
                    comments: builder.empty_comments(2),
                    l_par: Pos { line: 0, char: 8 },
                    values: vec![
                        InlineArrayValue {
                            comments: builder.empty_comments(3),
                            val: int(0, 9, "0"),
                            comma: Some(Pos { line: 0, char: 10 }),
                        },
                        InlineArrayValue {
                            comments: builder.empty_comments(3),
                            val: int(0, 12, "1"),
                            comma: None,
                        },
                        InlineArrayValue {
                            comments: builder.empty_comments(3),
                            val: int(0, 15, "2"),
                            comma: None,
                        },
                    ],
                    end: End::Par(Pos { line: 0, char: 16 }),
                }),
            ))]
        },
        Error::MissingComma(Pos { line: 0, char: 13 }),
    );
}

#[test]
fn inline_array_recover_invalid() {
    check_error(
        "array = [0, 1, 2, =]",
        |builder| {
            [Toplevel::Assignment(ta(
                builder.empty_comments(1),
                0,
                "array",
                Value::InlineArray(InlineArray {
                    comments: builder.empty_comments(2),
                    l_par: Pos { line: 0, char: 8 },
                    values: vec![
                        InlineArrayValue {
                            comments: builder.empty_comments(3),
                            val: int(0, 9, "0"),
                            comma: Some(Pos { line: 0, char: 10 }),
                        },
                        InlineArrayValue {
                            comments: builder.empty_comments(3),
                            val: int(0, 12, "1"),
                            comma: Some(Pos { line: 0, char: 13 }),
                        },
                        InlineArrayValue {
                            comments: builder.empty_comments(3),
                            val: int(0, 15, "2"),
                            comma: Some(Pos { line: 0, char: 16 }),
                        },
                    ],
                    end: End::Par(Pos { line: 0, char: 19 }),
                }),
            ))]
        },
        Error::ExpectedValueFound(
            "`=`".into(),
            Span::from_pos_len(Pos { line: 0, char: 18 }, 1),
        ),
    );
}

#[test]
fn nested_inline_array() {
    check("array = [[0], [1]]", |builder| {
        [Toplevel::Assignment(ta(
            builder.empty_comments(1),
            0,
            "array",
            Value::InlineArray(InlineArray {
                comments: builder.empty_comments(2),
                l_par: Pos { line: 0, char: 8 },
                values: vec![
                    InlineArrayValue {
                        comments: builder.empty_comments(3),
                        val: Value::InlineArray(InlineArray {
                            comments: builder.empty_comments(3),
                            l_par: Pos { line: 0, char: 9 },
                            values: vec![InlineArrayValue {
                                comments: builder.empty_comments(4),
                                val: int(0, 10, "0"),
                                comma: None,
                            }],
                            end: End::Par(Pos { line: 0, char: 11 }),
                        }),
                        comma: Some(Pos { line: 0, char: 12 }),
                    },
                    InlineArrayValue {
                        comments: builder.empty_comments(3),
                        val: Value::InlineArray(InlineArray {
                            comments: builder.empty_comments(3),
                            l_par: Pos { line: 0, char: 14 },
                            values: vec![InlineArrayValue {
                                comments: builder.empty_comments(4),
                                val: int(0, 15, "1"),
                                comma: None,
                            }],
                            end: End::Par(Pos { line: 0, char: 16 }),
                        }),
                        comma: None,
                    },
                ],
                end: End::Par(Pos { line: 0, char: 17 }),
            }),
        ))]
    });
}

#[test]
fn unclosed_inline_array_error_on_last_line() {
    let inputs = [
        "array = [\n  0,\n  1",   // without newline
        "array = [\n  0,\n  1\n", // with newline
    ];
    for input in inputs {
        check_error(
            input,
            |builder| {
                [Toplevel::Assignment(ta(
                    builder.empty_comments(1),
                    0,
                    "array",
                    Value::InlineArray(InlineArray {
                        comments: builder.empty_comments(2),
                        l_par: Pos { line: 0, char: 8 },
                        values: vec![
                            InlineArrayValue {
                                comments: builder.empty_comments(3),
                                val: int(1, 2, "0"),
                                comma: Some(Pos { line: 1, char: 3 }),
                            },
                            InlineArrayValue {
                                comments: builder.empty_comments(3),
                                val: int(2, 2, "1"),
                                comma: None,
                            },
                        ],
                        end: End::None(Pos { line: 2, char: 3 }),
                    }),
                ))]
            },
            Error::ExpectedRightSquareFound(
                "`EOF`".into(),
                Pos { line: 0, char: 8 },
                Span::pos(Pos { line: 2, char: 3 }),
            ),
        );
    }
}

#[test]
fn inline_table() {
    check("table = { a = 3, b = true }", |builder| {
        [Toplevel::Assignment(ta(
            builder.empty_comments(1),
            0,
            "table",
            Value::InlineTable(InlineTable {
                l_par: Pos { line: 0, char: 8 },
                assignments: vec![
                    InlineTableAssignment {
                        assignment: aint(0, 10, "a", "3"),
                        comma: Some(Pos { line: 0, char: 15 }),
                    },
                    InlineTableAssignment {
                        assignment: abool(0, 17, "b", true),
                        comma: None,
                    },
                ],
                end: End::Par(Pos { line: 0, char: 26 }),
            }),
        ))]
    });
}

#[test]
fn inline_table_recover_missing_comma() {
    check_error(
        "table = { a = 3  b = true }",
        |builder| {
            [Toplevel::Assignment(ta(
                builder.empty_comments(1),
                0,
                "table",
                Value::InlineTable(InlineTable {
                    l_par: Pos { line: 0, char: 8 },
                    assignments: vec![
                        InlineTableAssignment {
                            assignment: aint(0, 10, "a", "3"),
                            comma: None,
                        },
                        InlineTableAssignment {
                            assignment: abool(0, 17, "b", true),
                            comma: None,
                        },
                    ],
                    end: End::Par(Pos { line: 0, char: 26 }),
                }),
            ))]
        },
        Error::MissingComma(Pos { line: 0, char: 15 }),
    );
}

#[test]
fn inline_table_recover_invalid() {
    check_error(
        "table = { a = 3, b = true, = }",
        |builder| {
            [Toplevel::Assignment(ta(
                builder.empty_comments(1),
                0,
                "table",
                Value::InlineTable(InlineTable {
                    l_par: Pos { line: 0, char: 8 },
                    assignments: vec![
                        InlineTableAssignment {
                            assignment: aint(0, 10, "a", "3"),
                            comma: Some(Pos { line: 0, char: 15 }),
                        },
                        InlineTableAssignment {
                            assignment: abool(0, 17, "b", true),
                            comma: Some(Pos { line: 0, char: 25 }),
                        },
                    ],
                    end: End::Par(Pos { line: 0, char: 29 }),
                }),
            ))]
        },
        Error::ExpectedKeyFound(
            "`=`".into(),
            Span {
                start: Pos { line: 0, char: 27 },
                end: Pos { line: 0, char: 28 },
            },
        ),
    );
}

#[test]
fn table_header() {
    check("[my_table]\nentry = false\n", |builder| {
        [Toplevel::Table(Table {
            comments: builder.empty_comments(1),
            header: TableHeader::new(
                Pos { line: 0, char: 0 },
                Some(Key::One(Ident::from_plain_lit(
                    "my_table",
                    Span::from_pos_len(Pos { line: 0, char: 1 }, 8),
                ))),
                Some(Pos { line: 0, char: 9 }),
            ),
            assignments: vec![tabool(builder.ec(2), 1, "entry", false)],
        })]
    })
}

#[test]
fn array_header() {
    check("[[my_array]]\nentry = false\n", |builder| {
        [Toplevel::Array(ArrayEntry {
            comments: builder.empty_comments(1),
            header: ArrayHeader::new(
                (Pos { line: 0, char: 0 }, Pos { line: 0, char: 1 }),
                Some(Key::One(Ident::from_plain_lit(
                    "my_array",
                    Span::from_pos_len(Pos { line: 0, char: 2 }, 8),
                ))),
                (
                    Some(Pos { line: 0, char: 10 }),
                    Some(Pos { line: 0, char: 11 }),
                ),
            ),
            assignments: vec![tabool(builder.ec(2), 1, "entry", false)],
        })]
    })
}

#[test]
fn newline_is_required_after_table_header() {
    check_error(
        "[my_table]entry = false\n",
        |builder| {
            [Toplevel::Table(Table {
                comments: builder.empty_comments(1),
                header: TableHeader::new(
                    Pos { line: 0, char: 0 },
                    Some(Key::One(Ident::from_plain_lit(
                        "my_table",
                        Span::from_pos_len(Pos { line: 0, char: 1 }, 8),
                    ))),
                    Some(Pos { line: 0, char: 9 }),
                ),
                assignments: vec![twrap(builder.ec(2), abool(0, 10, "entry", false))],
            })]
        },
        Error::MissingNewline(Pos { line: 0, char: 10 }),
    )
}

#[test]
fn newline_is_required_after_assignment() {
    check_error(
        "a = false b = 87",
        |builder| {
            [
                Toplevel::Assignment(twrap(builder.ec(1), abool(0, 0, "a", false))),
                Toplevel::Assignment(twrap(builder.ec(1), aint(0, 10, "b", "87"))),
            ]
        },
        Error::MissingNewline(Pos { line: 0, char: 10 }),
    )
}

#[test]
fn table_header_with_associated_comment_above() {
    check("# associated\n[my_table]\n", |builder| {
        [Toplevel::Table(Table {
            comments: builder.comments(
                1,
                [AssocComment {
                    pos: AssocPos::Above,
                    level: 1,
                    comment: Comment {
                        span: Span::from_pos_len(Pos { line: 0, char: 0 }, 12),
                    },
                }],
            ),
            header: TableHeader::new(
                Pos { line: 1, char: 0 },
                Some(Key::One(Ident::from_plain_lit(
                    "my_table",
                    Span::from_pos_len(Pos { line: 1, char: 1 }, 8),
                ))),
                Some(Pos { line: 1, char: 9 }),
            ),
            assignments: Vec::new(),
        })]
    })
}

#[test]
fn non_associated_comment() {
    check("# free standing\n\n[my_table]\n", |builder| {
        builder.comments(
            0,
            [AssocComment {
                pos: AssocPos::Contained,
                level: 0,
                comment: Comment {
                    span: Span::from_pos_len(Pos { line: 0, char: 0 }, 15),
                },
            }],
        );

        [Toplevel::Table(Table {
            comments: builder.empty_comments(1),
            header: TableHeader::new(
                Pos { line: 2, char: 0 },
                Some(Key::One(Ident::from_plain_lit(
                    "my_table",
                    Span::from_pos_len(Pos { line: 2, char: 1 }, 8),
                ))),
                Some(Pos { line: 2, char: 9 }),
            ),
            assignments: Vec::new(),
        })]
    })
}

#[test]
fn comment_after_table_header() {
    check("[my_table] # comment\nentry = false\n", |builder| {
        [Toplevel::Table(Table {
            comments: builder.comments(
                1,
                [AssocComment {
                    pos: AssocPos::LineEnd,
                    level: 1,
                    comment: Comment {
                        span: Span::from_pos_len(Pos { line: 0, char: 11 }, 9),
                    },
                }],
            ),
            header: TableHeader::new(
                Pos { line: 0, char: 0 },
                Some(Key::One(Ident::from_plain_lit(
                    "my_table",
                    Span::from_pos_len(Pos { line: 0, char: 1 }, 8),
                ))),
                Some(Pos { line: 0, char: 9 }),
            ),
            assignments: vec![tabool(builder.ec(2), 1, "entry", false)],
        })]
    })
}

#[test]
fn associated_comments_above_assignment() {
    check("# comment 1\n# comment 2\nabc = false", |builder| {
        [Toplevel::Assignment(ToplevelAssignment {
            comments: builder.comments(
                1,
                [
                    AssocComment {
                        pos: AssocPos::Above,
                        level: 1,
                        comment: Comment {
                            span: Span::from_pos_len(Pos { line: 0, char: 0 }, 11),
                        },
                    },
                    AssocComment {
                        pos: AssocPos::Above,
                        level: 1,
                        comment: Comment {
                            span: Span::from_pos_len(Pos { line: 1, char: 0 }, 11),
                        },
                    },
                ],
            ),
            assignment: abool(2, 0, "abc", false),
        })]
    })
}

#[test]
fn comment_after_assignment() {
    check("abc = false # comment\n", |builder| {
        [Toplevel::Assignment(ToplevelAssignment {
            comments: builder.comments(
                1,
                [AssocComment {
                    pos: AssocPos::LineEnd,
                    level: 1,
                    comment: Comment {
                        span: Span::from_pos_len(Pos { line: 0, char: 12 }, 9),
                    },
                }],
            ),
            assignment: abool(0, 0, "abc", false),
        })]
    })
}

#[test]
fn comment_separated_by_blank_line_is_not_associated() {
    check("# free standing\n\n# associated\nabc = false", |builder| {
        builder.comments(
            1,
            [AssocComment {
                pos: AssocPos::Contained,
                level: 0,
                comment: Comment {
                    span: Span::from_pos_len(Pos { line: 0, char: 0 }, 15),
                },
            }],
        );

        [Toplevel::Assignment(ToplevelAssignment {
            comments: builder.comments(
                1,
                [AssocComment {
                    pos: AssocPos::Above,
                    level: 1,
                    comment: Comment {
                        span: Span::from_pos_len(Pos { line: 2, char: 0 }, 12),
                    },
                }],
            ),
            assignment: abool(3, 0, "abc", false),
        })]
    })
}

#[test]
fn comment_is_last_token() {
    check("abc = false\n# free standing", |builder| {
        let toplevel = [Toplevel::Assignment(tabool(builder.ec(1), 0, "abc", false))];
        builder.comments(
            0,
            [AssocComment {
                pos: AssocPos::Contained,
                level: 0,
                comment: Comment {
                    span: Span::from_pos_len(Pos { line: 1, char: 0 }, 15),
                },
            }],
        );

        toplevel
    })
}

#[test]
fn comment_contained_by_table() {
    check(
        "[my_table]\n\n# contained comment\n\nabc = false",
        |builder| {
            [Toplevel::Table(Table {
                comments: builder.comments(
                    1,
                    [AssocComment {
                        pos: AssocPos::Contained,
                        level: 1,
                        comment: Comment {
                            span: Span::from_pos_len(Pos { line: 2, char: 0 }, 19),
                        },
                    }],
                ),
                header: TableHeader::new(
                    Pos { line: 0, char: 0 },
                    Some(Key::One(Ident::from_plain_lit(
                        "my_table",
                        Span::from_pos_len(Pos { line: 0, char: 1 }, 8),
                    ))),
                    Some(Pos { line: 0, char: 9 }),
                ),
                assignments: vec![tabool(builder.ec(2), 4, "abc", false)],
            })]
        },
    )
}

#[test]
fn associated_comments_in_inline_array() {
    check(
        "\
array = [
    # comment 1
    # comment 2

    # above value
    1 # after value
    # contained comment
    , # after comma
    # comment 3
]",
        |builder| {
            let toplevel = [Toplevel::Assignment(ToplevelAssignment {
                comments: CommentRange::new(CommentId(0), 7, 1),
                assignment: a(
                    0,
                    0,
                    "array",
                    Value::InlineArray(InlineArray {
                        comments: {
                            builder.comments(
                                2,
                                [
                                    AssocComment {
                                        pos: AssocPos::Contained,
                                        level: 2,
                                        comment: Comment {
                                            span: Span::from_pos_len(Pos { line: 1, char: 4 }, 11),
                                        },
                                    },
                                    AssocComment {
                                        pos: AssocPos::Contained,
                                        level: 2,
                                        comment: Comment {
                                            span: Span::from_pos_len(Pos { line: 2, char: 4 }, 11),
                                        },
                                    },
                                ],
                            );

                            // last comment is added at the end
                            CommentRange::new(CommentId(0), 7, 2)
                        },
                        l_par: Pos { line: 0, char: 8 },
                        values: vec![InlineArrayValue {
                            comments: builder.comments(
                                3,
                                [
                                    AssocComment {
                                        pos: AssocPos::Above,
                                        level: 3,
                                        comment: Comment {
                                            span: Span::from_pos_len(Pos { line: 4, char: 4 }, 13),
                                        },
                                    },
                                    AssocComment {
                                        pos: AssocPos::LineEnd,
                                        level: 3,
                                        comment: Comment {
                                            span: Span::from_pos_len(Pos { line: 5, char: 6 }, 13),
                                        },
                                    },
                                    AssocComment {
                                        pos: AssocPos::Contained,
                                        level: 3,
                                        comment: Comment {
                                            span: Span::from_pos_len(Pos { line: 6, char: 4 }, 19),
                                        },
                                    },
                                    AssocComment {
                                        pos: AssocPos::LineEnd,
                                        level: 3,
                                        comment: Comment {
                                            span: Span::from_pos_len(Pos { line: 7, char: 6 }, 13),
                                        },
                                    },
                                ],
                            ),
                            val: int(5, 4, "1"),
                            comma: Some(Pos { line: 7, char: 4 }),
                        }],
                        end: End::Par(Pos { line: 9, char: 0 }),
                    }),
                ),
            })];

            // add last comment
            builder.comments(
                2,
                [AssocComment {
                    pos: AssocPos::Contained,
                    level: 2,
                    comment: Comment {
                        span: Span::from_pos_len(Pos { line: 8, char: 4 }, 11),
                    },
                }],
            );

            toplevel
        },
    );
}

#[test]
fn many_unclosed_inline_arrays() {
    _ = parse_simple("a=[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[");
}

#[test]
fn many_unclosed_inline_tables() {
    _ = parse_simple("a={{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{");
}

#[test]
fn recursion_limit_inline_array() {
    check_simple_error(
        "a=[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]",
        MapInner::new(),
        Error::RecursionLimitExceeded(Pos { line: 0, char: 100 }),
    );
}

#[test]
fn recursion_limit_inline_table() {
    check_simple_error(
        "a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a={a=}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}",
        MapInner::new(),
        Error::RecursionLimitExceeded(Pos { line: 0, char: 296 }),
    );
}

#[test]
fn offset_date_time_with_subsec() {
    check("abc = 2023-12-05T10:11:12.3324243-04:30", |builder| {
        [Toplevel::Assignment(ta(
            builder.empty_comments(1),
            0,
            "abc",
            Value::DateTime(DateTimeVal {
                lit_span: Span::from_pos_len(Pos { line: 0, char: 6 }, 33),
                val: DateTime::OffsetDateTime(
                    Date::new(2023, 12, 5),
                    Time::new(10, 11, 12, 332_424_300),
                    Offset::Custom(-(4 * 60 + 30)),
                ),
            }),
        ))]
    });
}

#[test]
fn offset_date_time_without_subsec() {
    check("abc = 2023-12-05T10:11:12+04:30", |builder| {
        [Toplevel::Assignment(ta(
            builder.empty_comments(1),
            0,
            "abc",
            Value::DateTime(DateTimeVal {
                lit_span: Span::from_pos_len(Pos { line: 0, char: 6 }, 25),
                val: DateTime::OffsetDateTime(
                    Date::new(2023, 12, 5),
                    Time::new(10, 11, 12, 0),
                    Offset::Custom(4 * 60 + 30),
                ),
            }),
        ))]
    });
}

#[test]
fn offset_date_time_with_z_suffix() {
    check("abc = 2023-12-05T10:11:12Z", |builder| {
        [Toplevel::Assignment(ta(
            builder.empty_comments(1),
            0,
            "abc",
            Value::DateTime(DateTimeVal {
                lit_span: Span::from_pos_len(Pos { line: 0, char: 6 }, 20),
                val: DateTime::OffsetDateTime(
                    Date::new(2023, 12, 5),
                    Time::new(10, 11, 12, 0),
                    Offset::Utc,
                ),
            }),
        ))]
    });
}

#[test]
fn space_separated_time() {
    check("abc = 2023-12-05 10:11:12", |builder| {
        [Toplevel::Assignment(ta(
            builder.empty_comments(1),
            0,
            "abc",
            Value::DateTime(DateTimeVal {
                lit_span: Span::from_pos_len(Pos { line: 0, char: 6 }, 19),
                val: DateTime::LocalDateTime(Date::new(2023, 12, 5), Time::new(10, 11, 12, 0)),
            }),
        ))]
    });
}

#[test]
fn local_date() {
    check("abc = 2023-12-05", |builder| {
        [Toplevel::Assignment(ta(
            builder.empty_comments(1),
            0,
            "abc",
            Value::DateTime(DateTimeVal {
                lit_span: Span::from_pos_len(Pos { line: 0, char: 6 }, 10),
                val: DateTime::LocalDate(Date::new(2023, 12, 5)),
            }),
        ))]
    });
}

#[test]
fn local_time() {
    check("abc = 10:11:12", |builder| {
        [Toplevel::Assignment(ta(
            builder.empty_comments(1),
            0,
            "abc",
            Value::DateTime(DateTimeVal {
                lit_span: Span::from_pos_len(Pos { line: 0, char: 6 }, 8),
                val: DateTime::LocalTime(Time::new(10, 11, 12, 0)),
            }),
        ))]
    });
}

#[test]
fn local_time_hour_out_of_range() {
    check_error(
        "abc = 25:00:00",
        |b| {
            [Toplevel::Assignment(tainvalid(
                b.ec(1),
                0,
                "abc",
                "25:00:00",
            ))]
        },
        Error::DateTimeOutOfBounds(
            DateTimeField::Hour,
            25,
            (0, 23),
            Span::from_pos_len(Pos { line: 0, char: 6 }, 2),
        ),
    );
}
