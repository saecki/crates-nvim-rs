...
use crate::{NvimCtx, NvimDiagnostics};
...
pub fn crates_nvim() -> nvim_oxi::Result<Dictionary> {
    let parse_toml = Function::from_fn::<_, nvim_oxi::Error>(move |()| {
        ...
        let container = Container::parse_with(&mut ctx, |bump| {
...
impl CrateBuilder {
    ...
    fn try_build(self, ctx: &mut impl NvimCtx) -> Option<TomlCrate> {
...
fn find(ctx: &mut impl NvimCtx, lines: &[impl AsRef<str>], map: &MapTable) -> Vec<TomlCrate> {
